...
export default function CatalogoPage() {
    ...
    const [usuarioLogueado, setUsuarioLogueado] = useState<{ nombre: string; rol: string } | null>(() => {
        ...
        return null; // Si no hay sesión, nace como null
    });
    useEffect(() => {
    ...
    // 2. Lógica al presionar el ícono de Perfil
    const handleProfileClick = () => {
        if (usuarioLogueado) {
        // Si ya inició sesión, abrimos/cerramos el dropdown de opciones
        setIsDropdownOpen(!isDropdownOpen);
        } else {
        // Si no ha iniciado sesión, abrimos el modal flotante
        setIsAuthModalOpen(true);
        }
    };

    // 3. Función para Cerrar Sesión (LOG OUT)
    const handleLogout = async () => {
        try {
        await authService.logout();
        setUsuarioLogueado(null);
        setIsDropdownOpen(false);
        window.location.reload(); // Recarga limpia para reiniciar estados de la app
        } catch {
    ...
    const agregarAlCarrito = (producto: Producto) => {
        ...
        setIsCartOpen(true); // Abre el carrito automáticamente para dar feedback
    };
    ...
    return (
        ...
        <header className="top-nav">
            ...
            <div className="profile-menu-container">
                ...
                {/* Si hay sesión iniciada, pinta su nombre al costado */}
